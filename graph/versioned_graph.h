...
struct versioned_graph {
  ...
  versioned_graph() {
    ...
    live_versions.insert(
        make_tuple(timestamp, make_tuple(refct_utils::make_refct(timestamp, 1), std::move(initial_graph))));
  }
  ...
  void release_version(version&& S) {
    ...
    if (refct_utils::get_rfct(pbbs::fetch_and_add(ref_ct_loc, -1)) == 2 && timestamp != latest_timestamp()) {
  ...
  void insert_edges_batch(size_t m, tuple<uintV, uintV>* edges, bool sorted = false, bool remove_dups = false,
    ...
    live_versions.insert(
        make_tuple(current_timestamp, make_tuple(refct_utils::make_refct(current_timestamp, 1), G_next.get_root())));
    G_next.clear_root();
