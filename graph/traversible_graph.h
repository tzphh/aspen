...
const flags no_dense =
    32;  // don't run dense (used to compare with systems that don't implement direction optimization)
...
struct traversable_graph : private graph {
  ...
  auto edge_map_dense(vertex_subset& vs, F f, pbbs::sequence<edge_struct> const& vtxs, const flags fl = 0) {
    ...
    if (should_output(fl)) {
      ...
      parallel_for(
          0, n, [&](size_t i) { next[i] = 0; }, (fl & run_sequential) ? std::numeric_limits<long>::max() : 0);
      parallel_for(
          0, n,
          [&](size_t i) {
            auto search_f = [&](uintV ngh) {
  ...
  template<class F>
  auto edge_map_sparse(vertex_subset& vs, F f, pbbs::sequence<uintE> const& offsets,
                       pbbs::sequence<edge_struct> const& vtxs, const flags fl = 0) {
    size_t m = vs.size();
    vs.to_sparse();
  ...
  template<class F>
  auto edge_map(vertex_subset& vs, F f, timer& sparse_t, timer& dense_t, timer& fetch_t, timer& other_t,
                const flags fl = 0) {
    size_t n = vs.n;
    size_t m = G::num_edges();
    size_t threshold = m / 20;
    if (vs.size() == 0) {
      return vertex_subset(n);
    }
  ...
  template<class F>
  auto edge_map(vertex_subset& vs, F f, pbbs::sequence<edge_struct> const& vtxs, timer& sparse_t, timer& dense_t,
                timer& other_t, const flags fl = 0) {
  ...
  traversable_graph insert_edges_batch(size_t m, tuple<uintV, uintV>* edges, bool sorted = false,
                                       bool remove_dups = false, size_t nn = std::numeric_limits<size_t>::max(),
                                       bool run_seq = false) const {
    return std::move(traversable_graph(G::insert_edges_batch(m, edges, sorted, remove_dups, nn, run_seq)));
  }

  traversable_graph insert_edges_batch(size_t vn, size_t vm, uintE* offsets, uintV* edges,
                                       size_t vertices_finished = 0) const {
    return traversable_graph(G::insert_edges_batch(vn, vm, offsets, edges, vertices_finished));
  }

  traversable_graph delete_edges_batch(size_t m, tuple<uintV, uintV>* edges, bool sorted = false,
                                       bool remove_dups = false, size_t nn = std::numeric_limits<size_t>::max(),
                                       bool run_seq = false) const {
