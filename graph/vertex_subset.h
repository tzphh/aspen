...
struct vertex_subset {
  ...
  // A sparse vertex set
  vertex_subset(size_t _n, size_t _m, uintV* _s) : n(_n), m(_m), s(_s), d(nullptr), is_dense(0) {}

  // A dense vertex set
  vertex_subset(size_t _n, size_t _m, bool* _d) : n(_n), m(_m), s(nullptr), d(_d), is_dense(1) {}

  // A dense vertex set
  vertex_subset(size_t _n, bool* _d) : n(_n), m(0), s(nullptr), d(_d), is_dense(1) {
...
void vertex_map(vertex_subset& vs, F f) {
  ...
  if (m > 0) {
    if (vs.is_dense) {
      ...
      parallel_for(0, m, [&](size_t i) { f(vs.s[i]); });
    }
  }
}
...
void add_to_vsubset(vertex_subset& vs, uintV* new_verts, uintV num_new_verts) {
  if (vs.is_dense) {
    parallel_for(0, num_new_verts, [&](size_t i) { vs.d[new_verts[i]] = true; });
    vs.m += num_new_verts;
  } else {
