...
auto build_large_compressed_graph(string fname, bool is_symmetric, size_t n_parts = 1) {
  ...
  while (vertices_finished < n) {
    ...
    debug(cout << endl; cout << "Inserting next batch, bs = " << bs << endl;);
...
auto initialize_graph(string fname = "", bool mmap = false, bool is_symmetric = true, bool compressed = false,
                      size_t n_parts = 1) {
  if (fname == "") {
